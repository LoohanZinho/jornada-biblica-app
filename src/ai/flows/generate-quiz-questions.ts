...
const AIPromptOutputSchema = z.object({
  questions: z.array(AIQuestionOutputSchema).describe('Um array de objetos de perguntas do quiz geradas pela IA.'),
});
...
const prompt = ai.definePrompt({
  ...
  output: {schema: AIPromptOutputSchema}, // IA gera de acordo com AIPromptOutputSchema
...
Para cada pergunta, forneça APENAS os seguintes campos:
...
5.  'imageHint': (Opcional, mas preferível) Uma ou duas palavras-chave EM INGLÊS que possam ser usadas para encontrar ou gerar uma imagem relevante para a pergunta (ex: "Moses staff", "Jerusalem temple", "dove peace"). Maximo de duas palavras.
6.  'hintText': (Opcional, mas PREFERÍVEL) Uma ÚNICA frase curta em português que sirva como uma pista SUTIL para a pergunta. NÃO deve revelar a resposta diretamente, mas guiar o pensamento. Ex: 'Este evento ocorreu perto de uma montanha famosa.' ou 'Este personagem era conhecido por sua grande força.' Evite dar a resposta na dica.

Certifique-se de que a resposta correta ('correctAnswer') seja uma das strings presentes no array 'options'.
Formate a saída como um objeto JSON que corresponda ao schema de saída especificado (contendo uma lista de 'questions' com os campos acima).
...
const generateQuizQuestionsFlow = ai.defineFlow(
  {
    ...
    outputSchema: GenerateQuizQuestionsOutputSchema, // O Flow retorna o schema completo
  },
  async (input: GenerateQuizQuestionsInput): Promise<GenerateQuizQuestionsOutput> => {
    const {output: aiOutput} = await prompt(input); // aiOutput está no formato AIPromptOutputSchema
    
    if (!aiOutput || !aiOutput.questions) {
      throw new Error('Falha ao gerar perguntas do quiz ou formato de saída inválido da IA.');
    }
    ...
    const processedQuestions: QuizQuestionType[] = aiOutput.questions.map((aiQuestion, index) => {
      return {
        ...aiQuestion, // Contém: question, options, correctAnswer, explanationContext, imageHint, hintText
        id: `gen-q${index + 1}-${Date.now()}`, // Adiciona o id
        topic: input.topic, // Adiciona o topic
        difficulty: input.difficulty, // Adiciona a difficulty
      };
    });
