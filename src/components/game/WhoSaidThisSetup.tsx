...
interface WhoSaidThisSetupProps {
  onStartGame: (settings: QuizSettings) => void;
}

export function WhoSaidThisSetup({ onStartGame }: WhoSaidThisSetupProps) {
  ...
  return (
    <Card className="w-full max-w-md mx-auto shadow-xl animate-fade-in">
      <CardHeader className="text-center">
        <QuoteIcon className="h-12 w-12 text-primary mx-auto mb-4" />
        <CardTitle className="text-3xl font-headline">Quem Disse Isso?</CardTitle>
        <CardDescription>Configure seu desafio e identifique os autores de citações bíblicas.</CardDescription>
      </CardHeader>
      <form onSubmit={handleSubmit(onSubmit)}>
        <CardContent className="space-y-6">
          <div className="space-y-2">
            <Label htmlFor="topic" className="font-semibold">Tópico das Citações</Label>
            <Input
              id="topic"
              aria-label="Digite o tópico para as citações"
              placeholder="Ex: Ensinos de Jesus, Profetas, Mulheres da Bíblia"
              {...register("topic")}
            />
          ...
          <div className="space-y-2">
            <Label htmlFor="numberOfQuestions" className="font-semibold">Número de Citações</Label>
            <Controller
              name="numberOfQuestions"
              control={control}
              render={({ field }) => (
                <Select onValueChange={(val) => field.onChange(parseInt(val))} defaultValue={String(field.value)}>
                  <SelectTrigger id="numberOfQuestions" aria-label="Selecionar número de citações">
